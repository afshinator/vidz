...
import { useTopicFilter, type TagGroup } from "@/hooks/use-topic-filter";
import type { ChannelWithTags } from "@/lib/db/queries";
import type { Tag } from "@/lib/db/schema";

/** Dummy tag used to construct a TagGroup for the untagged accordion section. */
const UNTAGGED_DUMMY_TAG: Tag = {
	id: "untagged",
	name: "Untagged",
	color: "#6b7280",
	userId: "",
...
function TopicAccordionSection({
	group,
	allTags,
	isUntagged,
}: {
		...
		return (
			<AccordionItem value="untagged" className="border rounded-lg px-4">
				...
				<AccordionContent className="pb-4 pt-1 [&_a]:no-underline">
					<ChannelGrid channels={group.channels} allTags={allTags} />
				</AccordionContent>
			</AccordionItem>
		);
	}
			...
			<AccordionTrigger className="hover:no-underline py-3 [&>svg]:shrink-0">
				<div className="flex items-center gap-2 flex-1 min-w-0 mr-2">
					<span
						...
						style={{ backgroundColor: group.tag.color || "#6366f1" }}
					/>
					<span className="text-sm font-semibold">{group.tag.name}</span>
					<span className="text-xs text-muted-foreground">
						({group.channels.length})
					</span>
					...
					<Button
						...
						onClick={(e) => {
							e.stopPropagation();
							startTransition(() => deleteTagAction(group.tag.id));
						}}
						title="Delete topic"
					>
...
export function TopicsClient({ channels, allTags }: TopicsClientProps) {
	const {
		...
		showUntagged,
		allSectionIds,
	} = useTopicFilter(channels, allTags);
	...
	return (
		<div>
			...
			<div className="flex flex-wrap gap-2 mb-6">
				...
				{allTags.map((tag) => {
					const group = tagGroups.find((g) => g.tag.id === tag.id);
					const count = group?.channels.length ?? 0;
					return (
						<FilterPill
							key={tag.id}
			...
			{visibleGroups.length > 0 || showUntagged ? (
				...
				>
					...
					{showUntagged && (
						<TopicAccordionSection
							group={{ tag: UNTAGGED_DUMMY_TAG, channels: untagged }}
							allTags={allTags}
							isUntagged
						/>
					)}
