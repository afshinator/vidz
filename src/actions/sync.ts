...
import {
	batchUpsertChannels,
	batchUpsertVideos,
	batchUpdateVideoCategoryIds,
	upsertSettings,
	upsertSyncState,
	getVideoIdsWithNullCategoryId,
} from "@/lib/db/queries";
import {
	buildCategoryUpdates,
...
function isQuotaExceeded(current: number, buffer = 10): boolean {
	return current >= QUOTA_LIMIT - buffer;
}
...
async function syncSubscriptions(
	...
	do {
		const { channels, nextPageToken } = await getSubscriptions(
			accessToken,
			pageToken,
		);
		quotaUsed.current += 1;
		...
		try {
			...
			errors.push(
				`Failed to save channels page: ${err instanceof Error ? err.message : "Unknown error"}`,
			);
		}

		pageToken = nextPageToken;
	} while (pageToken && !isQuotaExceeded(quotaUsed.current, 100));

	return { channelIds, channelsSynced, errors };
}

/** Sync videos for a single channel: paginate playlistItems, upsert, record sync state. */
	...
	try {
		...
		do {
			...
			try {
				...
				errors.push(
					`Failed to save videos for channel ${channelId}: ${err instanceof Error ? err.message : "Unknown error"}`,
				);
			}
		...
		if (err instanceof YouTubeQuotaError) throw err; // caller handles quota
		errors.push(
			`Failed to fetch videos for channel ${channelId}: ${err instanceof Error ? err.message : "Unknown error"}`,
		);
	}
...
async function syncChannelVideos(
	...
	for (const channelId of channelIds.slice(0, MAX_CHANNELS_PER_SYNC)) {
		if (isQuotaExceeded(quotaUsed.current)) break;

		try {
			const result = await syncSingleChannel(accessToken, channelId, quotaUsed);
			videosAdded += result.videosAdded;
			errors.push(...result.errors);
		} catch (err) {
			if (err instanceof YouTubeQuotaError) break;
			errors.push(
				`Failed for channel ${channelId}: ${err instanceof Error ? err.message : "Unknown error"}`,
			);
		}
	}
...
export async function syncNowAction(): Promise<SyncResult> {
	...
	try {
		...
		const videoResult = await syncChannelVideos(
			accessToken,
			subResult.channelIds,
			quotaUsed,
		);
...
export async function backfillCategoryIdsAction(): Promise<BackfillResult> {
	...
	for (let i = 0; i < videoIds.length; i += 50) {
		...
		const { updates, skipped: batchSkipped } = buildCategoryUpdates(
			batch,
			categoryMap,
		);
